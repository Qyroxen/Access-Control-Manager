...
> Access-Control-Manager - Open source tool by AetherCodeHQ

`access-control` `cli` `devops` `golang` `security`

## What is it?

**Access Control Manager** is a security tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
go build -o Access-Control-Manager .
```

### Run
```bash
./Access-Control-Manager --help
```
...
```bash
# Basic usage
./Access-Control-Manager --path ./target

# With options
./Access-Control-Manager --path ./target --format json --output report.json

# Verbose mode
./Access-Control-Manager --path ./target --verbose
```
...
### Basic scan
```bash
./Access-Control-Manager --path ./my-project
```

### JSON report
```bash
./Access-Control-Manager --path ./my-project --format json --output report.json
```

### CI/CD integration
```yaml
# .github/workflows/scan.yml
- name: Run Access Control Manager
  run: ./Access-Control-Manager --path . --format json --output report.json
```
